...
CXXFLAGS := -std=c++17 -fPIC -g -shared -fPIC -m64 -DMULTISIM_SW
CXX := g++

create_release:	multisim_client.so multisim_server.so copy_headers

copy_headers:
	echo "$(MULTISIM_HEADERS)" | tr ' ' '\n' | xargs -I{} cp {} $(RELEASE_DIR)

multisim_client.so: $(CLIENT_SRC) $(HEADERS) $(RELEASE_DIR)
	g++ $(CXXFLAGS) $(CLIENT_SRC) -o $(RELEASE_DIR)/$@


multisim_server.so: $(SERVER_SRC) $(HEADERS) $(RELEASE_DIR)
